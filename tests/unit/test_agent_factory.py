...
class TestAgentFactory:
    ...
    def test_create_web_agent_default(self):
        ...
        assert len(agent.agent.tools) >= 2  # Should have web_search and web_fetch at minimum
    ...
    def test_create_web_agent_with_custom_tools(self):
        ...
        tool_names = [tool.name if hasattr(tool, 'name') else tool for tool in agent.agent.tools]
        assert "dummy" in tool_names or any("dummy" in str(t).lower() for t in agent.agent.tools)
