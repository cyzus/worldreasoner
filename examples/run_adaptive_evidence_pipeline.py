...
def parse_args():
    ...
    parser.add_argument('--db', type=str, default='worldreasoner.db',
    ...
    parser.add_argument('--evidence-window', type=int, default=365,
                       help='Days before resolution to collect evidence (default: 365)')
    parser.add_argument('--min-resolution-age', type=int, default=0,
    ...
    # Processing settings
    parser.add_argument('--question-batch-size', type=int, default=10,
                       help='Batch size for evidence collection (default: 10)')
    parser.add_argument('--reasoning-batch-size', type=int, default=20,
...
async def show_graph_metrics(db_path: str, processed_questions):
    ...
    if depths:
        logger.info(f"Graph Depth by Question:")
        for question_id, depth, num_links in depths:
            status = "✓ DEEP" if depth >= 3 else "⚠ SHALLOW" if depth == 1 else "→ MODERATE"
        ...
        logger.info(f"  Deep graphs (3+ levels): {deep_graphs}/{len(depths)} ({100*deep_graphs/len(depths):.0f}%)")
        logger.info(f"  Shallow graphs (1 level): {shallow_graphs}/{len(depths)} ({100*shallow_graphs/len(depths):.0f}%)")
