...
class ForecastEvent(BaseModel):
    ...
    forecast_id: Optional[str] = Field(None, description="Forecast ID (set after submission)")
    session_id: str = Field(..., description="Session ID for tracking during forecasting")

    title: str = Field(..., description="Event title")
    description: str = Field(..., description="Event description")
    domain: Domain = Field(..., description="Event domain")
    occurred_date: Optional[datetime] = Field(None, description="When the event occurred")
    ...
    source_article_ids: List[str] = Field(default_factory=list, description="Article IDs supporting this event")
    identified_by: str = Field("forecast_agent", description="Who identified this event")
    created_at: datetime = Field(default_factory=lambda: datetime.now(timezone.utc), description="Creation timestamp")


@register_model('forecast_hypotheses', indexes=['forecast_id', 'session_id', 'source_event_id', 'target_event_id'])
class ForecastHypothesis(BaseModel):
    ...
    relation_type: CausalRelationType = Field(..., description="Type of causal relationship")

    strength: float = Field(..., ge=0.0, le=1.0, description="Relationship strength (0-1)")
    confidence: float = Field(..., ge=0.0, le=1.0, description="Confidence in hypothesis (0-1)")

    reasoning: str = Field(..., description="Explanation of the causal relationship")
    evidence_article_ids: List[str] = Field(default_factory=list, description="Article IDs supporting this hypothesis")

    identified_by: str = Field("forecast_agent", description="Who identified this hypothesis")
    created_at: datetime = Field(default_factory=lambda: datetime.now(timezone.utc), description="Creation timestamp")
