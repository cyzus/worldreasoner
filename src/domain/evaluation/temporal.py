...
class TemporalAnalyzer:
    ...
    def get_context_timeline(self, question: Question) -> List[Tuple[datetime, int, str]]:
        ...
        if question.target_event_id:
            event = self.db.get(Event, question.target_event_id)
            if event and event.occurred_date:
                timeline.append((ensure_aware(event.occurred_date), "event", event.id))
    ...
    def calculate_forecast_points(
        ...
        if len(valid_timeline) < min_context_items:
            raise ValueError(f"Insufficient context items ({len(valid_timeline)}) before resolution date.")

        # Find valid indices
        valid_indices = [
            i for i in range(len(valid_timeline))
        ...
        return [
            {
                ...
                "days_before_resolution": (question.resolution_date - valid_timeline[idx][0]).days
            }
