...
class RssFetchTool(Tool):
    ...
    def forward(self, feed_url: str, max_items: int = 10) -> str:
        ...
        if parsed.bozo:
            ...
            return json.dumps({"error": "Failed to parse feed", "feed_url": feed_url, "detail": err})
        ...
        return json.dumps({"feed_url": feed_url, "total_items": len(items), "items": items}, indent=2)
