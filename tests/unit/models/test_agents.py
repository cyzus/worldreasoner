...
class TestWebAgent:
    ...
    def test_web_agent_initialization(self):
        ...
        assert agent is not None
        assert agent.config is not None
        assert len(agent.agent.tools) >= 2  # Should have WebSearchTool and VisitWebpageTool
    ...
    def test_web_agent_has_web_tools(self):
        ...
        assert any("web" in str(name).lower() or "search" in str(name).lower() for name in tool_names)
