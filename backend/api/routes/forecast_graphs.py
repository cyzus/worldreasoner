...
async def get_forecast_graph(forecast_id: str):
    ...
    if not events and not hypotheses:
        ...
        if forecast and forecast.session_id:
            events = db.get_many(ForecastEvent, filters={'session_id': forecast.session_id})
            hypotheses = db.get_many(ForecastHypothesis, filters={'session_id': forecast.session_id})

    if not events and not hypotheses:
        raise HTTPException(
            status_code=404,
            detail=f"No graph data found for forecast {forecast_id}"
        )
    ...
    for e in events:
        event_dict = {
            ...
            'event_type': e.event_type.value if e.event_type and hasattr(e.event_type, 'value') else e.event_type if e.event_type else None,
