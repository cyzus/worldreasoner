...
def mock_assessments():
    ...
    return [
        QualityAssessment(question_id="q_001", composite_score=0.7, dimensions={}, reasoning=""),
        QualityAssessment(question_id="q_002", composite_score=0.9, dimensions={}, reasoning=""),
        QualityAssessment(question_id="q_003", composite_score=0.8, dimensions={}, reasoning=""),
    ]
    ...
    with patch.object(stage.scorer, 'forward', new_callable=MagicMock) as mock_forward:
        ...
        def side_effect(questions_batch):
            for q in questions_batch:
                assessment = next((a for a in mock_assessments if a.question_id == q.id), None)
...
async def test_question_not_in_assessment(sample_questions):
    ...
    mock_assessment = [QualityAssessment(question_id="q_001", composite_score=0.7, dimensions={}, reasoning="")]
