...
def sample_goal():
    ...
    return CollectionGoal(
        total_questions=100,
        type_distribution={"boolean": 50, "mcq": 30, "quantity": 20},
...
def test_gap_analysis_dataclass():
    ...
    analysis = GapAnalysis(
        type_gaps={"boolean": 10, "mcq": 5}, category_gaps={"tech": 8}, total_needed=23
    )

    assert analysis.has_gaps is True
    assert analysis.type_gaps_list == ["boolean", "mcq"]
...
def test_analyze_empty_progress(gap_analyzer, sample_goal):
    ...
    assert analysis.type_gaps == {"boolean": 50, "mcq": 30, "quantity": 20}
    assert analysis.category_gaps == {"tech": 40, "politics": 30, "economics": 30}
    assert analysis.has_gaps is True
...
def test_analyze_partial_progress(gap_analyzer, sample_goal):
    ...
    assert analysis.type_gaps["boolean"] == 25  # 50 - 25
...
def test_analyze_goal_met(gap_analyzer, sample_goal):
    ...
    for i in range(30):
        q = create_test_question(
            id=f"q_mcq_{i}",
            question_type="mcq",
            category="tech" if i < 15 else "economics",
            source_name="news",
        )
        progress.add_question(q)
    ...
    for i in range(20):
        q = create_test_question(
            ...
            category="economics" if i < 10 else "politics",
            source_name="news",
        )
        progress.add_question(q)
...
def test_analyze_filters_negative_gaps(gap_analyzer):
    ...
    goal = CollectionGoal(
        total_questions=50, type_distribution={"boolean": 25, "mcq": 25}
    )
    ...
    assert "boolean" not in analysis.type_gaps
    # MCQ under-collected
...
def test_type_gaps_list_property(gap_analyzer, sample_goal):
    ...
    assert set(analysis.type_gaps_list) == {"boolean", "mcq", "quantity"}
...
def test_category_gaps_list_property(gap_analyzer, sample_goal):
    ...
    assert set(analysis.category_gaps_list) == {"tech", "politics", "economics"}
