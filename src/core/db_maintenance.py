...
async def build_search_index(
    ...
    """Build (or rebuild) FTS5 + semantic search indexes for all articles.

    Args:
        db_path: Path to the database.
        embedding_model: LiteLLM embedding model. If None, read from config.
        rebuild: Clear existing indexes and rebuild from scratch.
        batch_size: Batch size for embedding generation.
        log: Optional progress callback (defaults to the module logger).
    """
    ...
    if embedding_model is None:
        ...
        emit(f"Using embedding model from config: {embedding_model}")

    emit(f"Building search index: db={db_path}, model={embedding_model}")

    db = GenericDatabase(db_path)
    search = HybridSearch(db_path, embedding_model=embedding_model)

    emit("Loading articles from database...")
    db.create_table(Article)
    articles = db.get_many(Article)
    if not articles:
        emit("No articles found in database. Nothing to index.")
        return

    emit(f"Found {len(articles)} articles")
    if rebuild:
        emit("Rebuilding indexes from scratch...")
        await search.reindex_all(articles)
    else:
        emit(f"Current index stats: {search.get_index_stats()}")
        await search.index_articles_batch(articles, batch_size=batch_size)

    final_stats = search.get_index_stats()
    emit("=" * 60)
    emit("Search Index Build Complete!")
    emit(f"Total articles in DB: {len(articles)}")
    emit(f"FTS5 indexed: {final_stats['fts_indexed']}")
    emit(f"Embeddings indexed: {final_stats['embeddings_indexed']}")
    emit(f"Embedding models: {final_stats['models']}")
    emit("=" * 60)
