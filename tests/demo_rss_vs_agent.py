...
async def demo_rss_approach():
    ...
    print(f"  Articles/second: {len(result.outputs)/duration:.2f}" if duration > 0 else "  Articles/second: N/A")
...
async def demo_agent_approach():
    ...
    print("\nConfiguration:")
    print(f"  Source: {sources[0].name}")
    print(f"  Type: Web Scraping with AI Agent")
    print(f"  URL: {sources[0].url}")
    print(f"  Max articles: 3")
    ...
    print(f"  Articles collected: {len(result.outputs)}")
    print(f"  Duration: {duration:.2f} seconds")
    print(f"  Articles/second: {len(result.outputs)/duration:.2f}" if duration > 0 and len(result.outputs) > 0 else "  Articles/second: N/A")
...
async def demo_comparison():
    ...
    if agent_skipped:
        print(f"│ Duration            │ {rss_time:>14.2f}s │ ~{agent_time:.0f}s (est)       │")
        print(f"│ Articles            │ {rss_count:>16} │ ~3 (est)         │")
        print(f"│ Articles/sec        │ {rss_count/rss_time:>16.2f} │ ~0.07 (est)      │")
        print(f"│ Speed ratio         │ {agent_time/rss_time:>14.1f}x │         1.0x     │")
    else:
        ...
        agent_rate = agent_count / agent_time if agent_time > 0 and agent_count > 0 else 0
        print(f"│ Articles/sec        │ {rss_rate:>16.2f} │ {agent_rate:>16.2f}" + (" │" if agent_rate > 0 else " │ N/A             │"))
    ...
    if not agent_skipped and rss_time < agent_time:
        ...
        print(f"  ℹ️  Agent test skipped (no API key) - using estimates")
