...
def parse_args():
    ...
    parser.add_argument(
        "--dpi", type=int, default=150, help="DPI for saved figures (default: 150)"
    )
...
def extract_dataframe(benchmarks: List[Dict[str, Any]]) -> pd.DataFrame:
    ...
    for bench in benchmarks:
        ...
        record = {
            ...
            "mode": bench["model_info"].get("mode")
            or (
                "knowledge_only"
                if bench["model_info"].get("knowledge_only")
                else "container"
            ),
            "mode_label": bench["model_info"]
            .get(
                "mode",
                "Knowledge-Only"
                if bench["model_info"].get("knowledge_only")
...
def plot_model_accuracy_comparison(df: pd.DataFrame, output_path: Path = None):
    ...
    # Color by mode
    colors = ["#3498db" if mode == "Full" else "#e74c3c" for mode in grouped["mode"]]
    ...
    ax.set_title(
        "Model Accuracy Comparison (Most Recent Run)", fontsize=13, fontweight="bold"
    )
...
def plot_brier_score_comparison(df: pd.DataFrame, output_path: Path = None):
    ...
    ax.set_title(
        "Model Brier Score Comparison (Most Recent Run)", fontsize=13, fontweight="bold"
    )
    ax.grid(axis="x", alpha=0.3)
...
def plot_mode_comparison(df: pd.DataFrame, output_path: Path = None):
    ...
    fig, (ax1, ax2) = plt.subplots(
        1, 2, figsize=(14, max(6, len(models_with_both) * 0.6))
    )
    ...
    for i, model in enumerate(models_with_both):
        ...
        knowledge = model_data[model_data["mode"] == "Knowledge-Only"][
            "accuracy"
        ].values[0]

        ax1.barh(
            i - 0.2, full * 100, 0.4, label="Full" if i == 0 else "", color="#3498db"
        )
        ax1.barh(
            i + 0.2,
            knowledge * 100,
            0.4,
            label="Knowledge-Only" if i == 0 else "",
            color="#e74c3c",
        )
    ...
    if has_brier:
        for i, model in enumerate(models_with_both):
            model_data = comp_df[comp_df["model"] == model]
            full_brier = model_data[model_data["mode"] == "Full"]["brier_score"].values[
                0
            ]
            knowledge_brier = model_data[model_data["mode"] == "Knowledge-Only"][
                "brier_score"
            ].values[0]
            ...
            if pd.notna(full_brier) and pd.notna(knowledge_brier):
                ...
                ax2.text(
                    max(full_brier, knowledge_brier) + 0.01,
                    i,
                    f"{diff:.4f}",
                    va="center",
                    fontsize=8,
        ...
        ax2.set_title(
            "Brier Score: Full vs Knowledge-Only", fontsize=12, fontweight="bold"
        )
    ...
    plt.suptitle(
        "Research Impact: Full Mode vs Knowledge-Only Mode",
        fontsize=14,
        fontweight="bold",
        y=1.02,
    )
...
def plot_log_score_comparison(df: pd.DataFrame, output_path: Path = None):
    ...
    ax.set_title(
        "Model Log Score Comparison (Most Recent Run)", fontsize=13, fontweight="bold"
    )
    ax.grid(axis="x", alpha=0.3)
    ...
    for i, (bar, score) in enumerate(zip(bars, grouped["log_score"])):
        ...
        ax.text(score + offset, i, f"{score:.4f}", va="center", ha=ha, fontsize=9)
